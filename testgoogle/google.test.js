...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.css("#APjFqb")).sendKeys("devmountain", Key.RETURN)
  await driver.wait(until.titleIs("devmountain - Google Search"))
  // TODO Call .clear() on the search bar element to clear the old search term
  await driver.findElement(By.css("#APjFqb")).clear()
  // TODO Call .sendKeys() on the search bar element to search for a new term
  await driver.findElement(By.css("#APjFqb")).sendKeys("javascript", Key.RETURN)
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("javascript - Google Search"))
});
